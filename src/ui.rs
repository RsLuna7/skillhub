use crate::config::AppConfig;
use crate::db::Database;
use crate::trust::TrustStatus;
use crate::{audit, scan, search, trust};
use anyhow::Result;
use askama::Template;
use axum::Router;
use axum::extract::{Path, Query, State};
use axum::http::StatusCode;
use axum::http::header;
use axum::response::{Html, IntoResponse, Redirect, Response};
use axum::routing::{get, post};
use serde::Deserialize;
use std::collections::BTreeMap;
use std::path::PathBuf;

const UI_CSS: &str = include_str!("../assets/ui.css");

#[derive(Debug, Clone)]
pub struct UiState {
    pub cfg: AppConfig,
    pub db_path: PathBuf,
}

pub fn run_ui(_cfg: AppConfig, port: u16, open: bool) -> Result<()> {
    let cfg = _cfg;
    let url = format!("http://127.0.0.1:{port}");
    if open {
        let _ = webbrowser::open(&url);
    }
    println!("SkillHub UI: {url}");
    let state = UiState {
        db_path: cfg.index_path(),
        cfg,
    };
    let runtime = tokio::runtime::Runtime::new()?;
    runtime.block_on(async move {
        let listener = tokio::net::TcpListener::bind(("127.0.0.1", port)).await?;
        axum::serve(listener, router(state)).await?;
        Ok::<(), anyhow::Error>(())
    })
}

pub fn router(state: UiState) -> Router {
    Router::new()
        .route("/", get(dashboard))
        .route("/skills", get(skills_list))
        .route("/skills/{skill_id}", get(skill_detail))
        .route("/scan", post(scan_incremental))
        .route("/scan/force", post(scan_force))
        .route("/skills/{skill_id}/audit", post(audit_one))
        .route("/skills/{skill_id}/trust/allow", post(trust_allow))
        .route("/skills/{skill_id}/trust/block", post(trust_block))
        .route("/skills/{skill_id}/trust/reset", post(trust_reset))
        .route("/assets/ui.css", get(css))
        .with_state(state)
}

#[derive(Debug, Clone)]
pub struct CountView {
    label: String,
    count: usize,
}

#[derive(Template)]
#[template(path = "ui/dashboard.html")]
struct DashboardTemplate {
    notice: Option<String>,
    total_skills: usize,
    source_counts: Vec<CountView>,
    risk_counts: Vec<CountView>,
    trust_counts: Vec<CountView>,
}

#[derive(Debug, Deserialize, Default)]
struct DashboardParams {
    notice: Option<String>,
}

#[derive(Debug, Clone)]
struct SkillRow {
    id: String,
    name: String,
    summary: String,
    risk: String,
    source_agent: String,
    trust: String,
    visibility: String,
}

#[derive(Debug, Deserialize, Default)]
struct SkillFilters {
    q: Option<String>,
    source: Option<String>,
    risk: Option<String>,
    trust: Option<String>,
}

#[derive(Template)]
#[template(path = "ui/skills.html")]
struct SkillsTemplate {
    filters: SkillFilters,
    skills: Vec<SkillRow>,
}

#[derive(Debug, Clone)]
struct SourceView {
    agent: String,
    root: String,
    priority: u8,
}

#[derive(Template)]
#[template(path = "ui/skill_detail.html")]
struct SkillDetailTemplate {
    summary: crate::skill::SkillUsageSummary,
    sources: Vec<SourceView>,
}

async fn dashboard(
    State(state): State<UiState>,
    Query(params): Query<DashboardParams>,
) -> Result<Html<String>, (StatusCode, String)> {
    let db = open_db(&state).map_err(ui_error)?;
    let skills = db.list_skills().map_err(ui_error)?;
    let mut source_counts = BTreeMap::<String, usize>::new();
    let mut risk_counts = BTreeMap::<String, usize>::new();
    let mut trust_counts = BTreeMap::<String, usize>::new();

    for skill in &skills {
        let source = if skill.source_agent.is_empty() {
            "unknown".to_string()
        } else {
            skill.source_agent.clone()
        };
        *source_counts.entry(source).or_default() += 1;
        *risk_counts
            .entry(skill.risk_level.as_str().to_string())
            .or_default() += 1;
        let trust = db
            .get_trust(&skill.id)
            .map_err(ui_error)?
            .map(|record| record.status)
            .unwrap_or(TrustStatus::Untrusted);
        *trust_counts.entry(trust.as_str().to_string()).or_default() += 1;
    }

    render(DashboardTemplate {
        notice: params.notice,
        total_skills: skills.len(),
        source_counts: count_views(source_counts),
        risk_counts: count_views(risk_counts),
        trust_counts: count_views(trust_counts),
    })
}

async fn skills_list(
    State(state): State<UiState>,
    Query(filters): Query<SkillFilters>,
) -> Result<Html<String>, (StatusCode, String)> {
    let db = open_db(&state).map_err(ui_error)?;
    let mut rows = Vec::new();
    for skill in db.list_skills().map_err(ui_error)? {
        let trust_status = trust_status_for(&db, &skill.id)?;
        let row = SkillRow {
            id: skill.id.clone(),
            name: skill.name.clone(),
            summary: skill.summary.clone(),
            risk: skill.risk_level.as_str().to_string(),
            source_agent: display_source(&skill.source_agent),
            visibility: trust::visibility_for(&trust_status).as_str().to_string(),
            trust: trust_status.as_str().to_string(),
        };
        if matches_filters(&row, &skill.detected_capabilities, &filters) {
            rows.push(row);
        }
    }
    render(SkillsTemplate {
        filters,
        skills: rows,
    })
}

async fn skill_detail(
    State(state): State<UiState>,
    Path(skill_id): Path<String>,
) -> Result<Html<String>, (StatusCode, String)> {
    let db = open_db(&state).map_err(ui_error)?;
    if db.get_skill(&skill_id).map_err(ui_error)?.is_none() {
        return Err((
            StatusCode::NOT_FOUND,
            format!("skill not found: {skill_id}"),
        ));
    }
    let summary = search::usage_summary(&db, &skill_id).map_err(ui_error)?;
    let sources = db
        .get_skill_sources(&skill_id)
        .map_err(ui_error)?
        .into_iter()
        .map(|(agent, root, priority)| SourceView {
            agent,
            root,
            priority,
        })
        .collect();
    render(SkillDetailTemplate { summary, sources })
}

async fn scan_incremental(State(state): State<UiState>) -> Result<Redirect, (StatusCode, String)> {
    let report = run_scan(&state, false)?;
    Ok(Redirect::to(&scan_notice_url(&report)))
}

async fn scan_force(State(state): State<UiState>) -> Result<Redirect, (StatusCode, String)> {
    let report = run_scan(&state, true)?;
    Ok(Redirect::to(&scan_notice_url(&report)))
}

async fn audit_one(
    State(state): State<UiState>,
    Path(skill_id): Path<String>,
) -> Result<Redirect, (StatusCode, String)> {
    let db = open_db(&state).map_err(ui_error)?;
    audit::audit_skill(&db, &skill_id).map_err(ui_error)?;
    Ok(Redirect::to(&format!("/skills/{skill_id}")))
}

async fn trust_allow(
    State(state): State<UiState>,
    Path(skill_id): Path<String>,
) -> Result<Redirect, (StatusCode, String)> {
    let db = open_db(&state).map_err(ui_error)?;
    trust::allow(&db, &skill_id).map_err(ui_error)?;
    Ok(Redirect::to(&format!("/skills/{skill_id}")))
}

async fn trust_block(
    State(state): State<UiState>,
    Path(skill_id): Path<String>,
) -> Result<Redirect, (StatusCode, String)> {
    let db = open_db(&state).map_err(ui_error)?;
    trust::block(&db, &skill_id, Some("blocked from SkillHub UI".to_string())).map_err(ui_error)?;
    Ok(Redirect::to(&format!("/skills/{skill_id}")))
}

async fn trust_reset(
    State(state): State<UiState>,
    Path(skill_id): Path<String>,
) -> Result<Redirect, (StatusCode, String)> {
    let db = open_db(&state).map_err(ui_error)?;
    trust::reset(&db, &skill_id).map_err(ui_error)?;
    Ok(Redirect::to(&format!("/skills/{skill_id}")))
}

async fn css() -> Response {
    ([(header::CONTENT_TYPE, "text/css; charset=utf-8")], UI_CSS).into_response()
}

fn open_db(state: &UiState) -> Result<Database> {
    let db = Database::open(&state.cfg)?;
    db.migrate()?;
    Ok(db)
}

fn run_scan(state: &UiState, force: bool) -> Result<scan::ScanReport, (StatusCode, String)> {
    let db = open_db(state).map_err(ui_error)?;
    let home = crate::config::home_dir();
    let cwd = std::env::current_dir().unwrap_or_else(|_| home.clone());
    let roots = state.cfg.discovered_roots(&home, &cwd);
    scan::scan_roots(&state.cfg, &db, &roots, force).map_err(ui_error)
}

fn scan_notice_url(report: &scan::ScanReport) -> String {
    format!(
        "/?notice=Scanned%20{}%20roots%2C%20indexed%20{}%20skills",
        report.roots_scanned, report.skills_indexed
    )
}

fn count_views(counts: BTreeMap<String, usize>) -> Vec<CountView> {
    counts
        .into_iter()
        .map(|(label, count)| CountView { label, count })
        .collect()
}

fn trust_status_for(db: &Database, skill_id: &str) -> Result<TrustStatus, (StatusCode, String)> {
    db.get_trust(skill_id)
        .map_err(ui_error)
        .map(|record| record.map(|r| r.status).unwrap_or(TrustStatus::Untrusted))
}

fn display_source(source_agent: &str) -> String {
    if source_agent.is_empty() {
        "unknown".to_string()
    } else {
        source_agent.to_string()
    }
}

fn matches_filters(row: &SkillRow, capabilities: &[String], filters: &SkillFilters) -> bool {
    if let Some(query) = filters.q.as_deref().filter(|q| !q.trim().is_empty()) {
        let needle = query.to_lowercase();
        let haystack = format!(
            "{} {} {} {}",
            row.id,
            row.name,
            row.summary,
            capabilities.join(" ")
        )
        .to_lowercase();
        if !haystack.contains(&needle) {
            return false;
        }
    }
    if let Some(source) = filters.source.as_deref().filter(|s| !s.trim().is_empty())
        && row.source_agent != source
    {
        return false;
    }
    if let Some(risk) = filters.risk.as_deref().filter(|r| !r.trim().is_empty())
        && row.risk != risk
    {
        return false;
    }
    if let Some(trust) = filters.trust.as_deref().filter(|t| !t.trim().is_empty())
        && row.trust != trust
    {
        return false;
    }
    true
}

fn render<T: Template>(template: T) -> Result<Html<String>, (StatusCode, String)> {
    template.render().map(Html).map_err(ui_error)
}

fn ui_error(error: impl std::fmt::Display) -> (StatusCode, String) {
    (StatusCode::INTERNAL_SERVER_ERROR, error.to_string())
}
